...
private:
    ...
    void handleDescribe() {
        ...
        snprintf(sdp, sizeof(sdp),
            ...
            "c=IN IP4 0.0.0.0\r\n"
            "a=control:trackID=0\r\n",
            sessionId, WiFi.localIP().toString().c_str(), RTSP_PORT);
...
public:
    RTSPServer() : server(RTSP_PORT), clientConnected(false), 
                   sessionId(random(1000000)), sequenceNumber(0), timestamp(0) {}
    ...
    void handle() {
        ...
        if (clientConnected && client.available()) {
            ...
            // Parse request
            if (request.indexOf("OPTIONS") >= 0) handleOptions();
            else if (request.indexOf("DESCRIBE") >= 0) handleDescribe();
            else if (request.indexOf("SETUP") >= 0) handleSetup();
            else if (request.indexOf("PLAY") >= 0) handlePlay();
            else if (request.indexOf("TEARDOWN") >= 0) handleTeardown();
