...
class ESPCamBenchmark:
    ...
    def capture_video(self, duration: int, output_path: str) -> Dict[str, Any]:
        ...
        self.logger.info(
            "Video properties: %dx%d @ %d fps", frame_width, frame_height, fps
        )
        ...
        for second in sorted_seconds:
            if (
                second > 0 and second <= duration
            ):  # Используем только полные секунды в пределах duration
        ...
        # Calculate FPS percentiles (минимальный FPS, в который укладывается N%
        # секунд)
        if complete_seconds_fps:
            complete_seconds_fps.sort(reverse=True)  # Сортируем по убыванию
            fps_percentiles = {
                "p50": complete_seconds_fps[
                    len(complete_seconds_fps) // 2
                ],  # 50% секунд имеют FPS не ниже этого значения
                "p75": complete_seconds_fps[
                    int(len(complete_seconds_fps) * 0.75)
                ],  # 75% секунд имеют FPS не ниже этого значения
                "p90": complete_seconds_fps[
                    int(len(complete_seconds_fps) * 0.9)
                ],  # 90% секунд имеют FPS не ниже этого значения
                "p95": complete_seconds_fps[
                    int(len(complete_seconds_fps) * 0.95)
                ],  # 95% секунд имеют FPS не ниже этого значения
                "p99": complete_seconds_fps[
                    int(len(complete_seconds_fps) * 0.99)
                ],  # 99% секунд имеют FPS не ниже этого значения
            }
        else:
        ...
        fps_stats = {
            ...
            "fps_stability": (
                round(statistics.stdev(complete_seconds_fps), 2)
        ...
        metrics.update(
            {
                ...
                "avg_fps": len(complete_seconds_fps) > 0
                and sum(complete_seconds_fps) / len(complete_seconds_fps)
                or 0,
                ...
                "bitrate_mbps": (
                    (file_size * 8) / (test_duration * 1024 * 1024)
                    if test_duration > 0
                    else 0
                ),
                "test_duration": test_duration,
                # Duration of analyzed data (excluding incomplete seconds)
    ...
    def _generate_file_name(
        self, test_params: Dict[str, Any], file_type: str, extension: str
    ) -> str:
    ...
    def _save_results(self, results: Dict[str, Any]) -> None:
        ...
        metrics_file = metrics_dir / self._generate_file_name(
            results["params"], "metrics", "json"
        )
    ...
    def run_test_combination(
        ...
        log_file = Path("results/logs") / self._generate_file_name(
            test_params, "log", "log"
        )
        ...
        try:
            ...
            if test_params.get("video_protocol"):
                video_path = Path("results/video") / self._generate_file_name(
                    test_params, "video", "mp4"
                )
                video_path.parent.mkdir(parents=True, exist_ok=True)
                results["video_metrics"] = self.capture_video(
                    self.config["test_duration"], str(video_path)
                )
            ...
            self.logger.info(
                "Test completed successfully in %.2f seconds",
                results["system_metrics"]["total_duration"],
            )
            self.logger.info("System metrics:")
            if not skip_build:
                self.logger.info(
                    "  Build time: %.2f seconds",
                    results["system_metrics"]["build_time"],
                )
                self.logger.info(
                    "  Flash time: %.2f seconds",
                    results["system_metrics"]["flash_time"],
                )
            self.logger.info(
                "  Boot time: %.2f seconds", results["system_metrics"]["boot_time"]
            )
